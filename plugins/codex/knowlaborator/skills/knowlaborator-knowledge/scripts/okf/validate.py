...
2 = bad invocation / incomplete validator package.

Uses the pinned pure-Python PyYAML copy bundled beside this script. It does not
install packages or use a host-provided PyYAML distribution.
"""
...
def main(argv=None):
    ...
    if yaml.__version__ != EXPECTED_PYYAML_VERSION:
        print(
            "error: bundled PyYAML version mismatch: "
            f"expected {EXPECTED_PYYAML_VERSION}, found {yaml.__version__}",
            file=sys.stderr,
        )
        return 2

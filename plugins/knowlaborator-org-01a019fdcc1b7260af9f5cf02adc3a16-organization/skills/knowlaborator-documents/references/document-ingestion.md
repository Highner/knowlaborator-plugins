...
8. Use `get_document_version` to check processing. When the saved bytes must be
   ...
   exact absolute path returned in `localFiles` with ordinary Codex local
   file/document tools. Never use indexed chunks, preview text, or metadata as
...
1. Wait until the saved version is the Document's exact `currentVersionId`,
   ...
   inspect the materialized original at the returned `localFiles` path.

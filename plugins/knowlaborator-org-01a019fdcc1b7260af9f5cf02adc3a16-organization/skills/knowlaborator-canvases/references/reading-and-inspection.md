...
2. Call `get_canvas` for the exact selected canvas. It returns metadata, the
   current `revision`, the stored lenses, `canonicalDraft`
   (`distillationId`, `status`, `sourceCanvasRevision`, `updatedAt`,
   `failureCode`), `automationEnabled`, `capabilities` and `archivedAt`.
...
- `creatorActorKind` and `lastEditorActorKind` are `member`, `agent` or
  `automation`.
...
Companion `localFiles` handoff and handling outside Companion. Inspect the
...
`list_canvas_distillations` pages the saved manual artifacts and the canonical
draft (`kind` `manual` or `canonical`, `scope` `canvas`, `lens` or
`selection`, `draftStatus`, `sourceCanvasRevision`). `get_canvas_distillation`
